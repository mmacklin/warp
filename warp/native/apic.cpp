...
// Platform-independent APIC (API Capture) implementation.
// CUDA-only functions live in apic.cu (included at the end of warp.cu).
...
// Out-of-line destructor (declared in apic.h, defined here because it
// references wp_mesh_destroy_device and CUDA runtime symbols from warp.h).
...
// Note: regions may be in device memory (CUDA) or host memory (CPU)
